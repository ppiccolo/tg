...
void generate_prompt(enum tgl_value_type type, int num) {
    switch (type) {
        case tgl_phone_number:
            assert(!num);
            one_string_prompt = tstrdup("phone number1: ");
            one_string_flags = 0;
            return;
        case tgl_code:
            assert(!num);
...
void read_secret_chat_file(void) {
    ...
    //assert (secret_chat_fd >= 0);
    int x;
    if (read(secret_chat_fd, &x, 4) < 4) {
        close(secret_chat_fd);
        return;
    }
    if (x != SECRET_CHAT_FILE_MAGIC) {
        close(secret_chat_fd);
        return;
    }
    int v = 0;
...
void clist_cb(struct tgl_state *TLSR, void *callback_extra, int success, int size, tgl_peer_id_t peers[],
              tgl_message_id_t *last_msg_id[], int unread_count[]) {
    on_started(TLS);
}

void dlist_cb(struct tgl_state *TLSR, void *callback_extra, int success, int size, tgl_peer_id_t peers[],
              tgl_message_id_t *last_msg_id[], int unread_count[]) {
