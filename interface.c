...
void do_phone_register (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
    assert (arg_num == 1);
    if (ev) {
        mprint_start (ev);
    }

    int phone_len = strlen (args[0].str);
...
void do_send_code (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
    assert (arg_num == 1);
    assert (global_registration_values != NULL);
    if (ev) {
        mprint_start (ev);
    }

    int code_len = strlen (args[0].str);
    const char *code[code_len];
    code[0] = args[0].str;
...
void do_get_registration_status (struct command *command, int arg_num, struct arg args[], struct in_ev *ev) {
    assert (!arg_num);
    if (ev) {
        mprint_start (ev);
    }
...
struct command commands[MAX_COMMANDS_SIZE] = {
  ...
  {"version", {ca_none}, do_version, "version\tPrints client and library version", NULL},
    // PP add reg function over socket to register new phone via API
  {"phone_register", {ca_string}, do_phone_register, "Register phone number", NULL},
  {"send_code", {ca_string}, do_send_code, "Send registration code to complete authentication", NULL},
  {"get_registration_status", {ca_none}, do_get_registration_status, "Get back the current registration step status", NULL},
};
